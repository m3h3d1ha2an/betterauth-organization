...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // Using Cookie Store API with a fallback
      if (typeof cookieStore !== "undefined") {
        cookieStore.set({
          name: SIDEBAR_COOKIE_NAME,
          value: String(openState),
          path: "/",
          // Note: expires uses a timestamp, max-age uses seconds
          expires: Date.now() + SIDEBAR_COOKIE_MAX_AGE * 1000,
        });
      } else {
        // Fallback for older browsers or ignore lint
        // biome-ignore lint/suspicious/noDocumentCookie: cookieStore is not fully supported yet
